...
impl Kurinji
{
    pub(crate) fn action_event_producer(input_map: Res<Kurinji>,
                                        ...
                                        mut on_end_event: ResMut<Events<OnActionEnd>>)
    {
        ...
        for (action, strength) in _actions
        {
            ...
            {
                on_active_event.send(OnActionActive { action:
                                                          action.clone(),
                                                      strength });
            }
            if input_map.did_action_just_began(&action)
            {
                on_begin_event.send(OnActionBegin { action: action.clone(),
                                                    strength });
            }
            if input_map.is_action_in_progress(&action)
            {
                on_progress_event.send(OnActionProgress { action:
                                                              action.clone(),
                                                          strength });
            }
